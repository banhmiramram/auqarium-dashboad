...
void tft_fill_rect(int x, int y, int w, int h, uint16_t color)
{
    for(int i = x; i < x + w; i++) {
        for(int j = y; j < y + h; j++) {
            tft_draw_pixel(i, j, color); // dùng hàm đã có vẽ pixel
        }
    }
}
...
{
    static float prev_temp = -999, prev_ph = -999, prev_tds = -999, prev_turb = -999;
    static int initialized = 0;

    // --- Màu sắc ---
    const uint16_t bg_color     = 0x0000;
    const uint16_t white        = 0xFFFF;
    const uint16_t blue_light   = 0x5D9B;
    const uint16_t blue_icon    = 0x07FF;
    const uint16_t LIGHT_BLUE   = 0x5DDF;

    // --- Kích thước & bố cục ---
    const int screen_w     = 240;
    const int scale_title  = 2;
    const int scale_data   = 2;
    const int frame_x      = 15, frame_y = 50, frame_w = 210, frame_h = 240;
    const int line_h       = 45;
    const int text_x       = frame_x + 35;
    const int start_y      = frame_y + 25;

    char buf[32];

    // --- Vẽ khung, tiêu đề, icon, footer lần đầu ---
    if (!initialized) {
        initialized = 1;
        ...
        // Icon cố định
        tft_draw_circle(frame_x + 15, start_y + 6, 4, blue_icon);            // TEMP
        tft_draw_filled_circle(frame_x + 15, start_y + line_h + 6, 3, blue_icon); // PH
        tft_draw_triangle(frame_x + 12, start_y + 2*line_h + 10,
                          frame_x + 15, start_y + 2*line_h + 3,
                          frame_x + 18, start_y + 2*line_h + 10, blue_icon); // TDS
        tft_draw_circle(frame_x + 15, start_y + 3*line_h + 6, 3, blue_icon); // TURB
    ...
    // --- Cập nhật TEMP ---
    if (fabs(temp - prev_temp) >= 0.01) {
        int rect_w = frame_w - (text_x - frame_x); // đủ rộng khung hiển thị chữ
        int rect_h = 18; // chiều cao font scale 2
        tft_fill_rect(text_x, start_y - 1, rect_w, rect_h, bg_color);
    ...
    // --- Cập nhật PH ---
    if (fabs(ph - prev_ph) >= 0.01) {
        int rect_w = frame_w - (text_x - frame_x);
        int rect_h = 18;
        tft_fill_rect(text_x, start_y + line_h - 1, rect_w, rect_h, bg_color);
    ...
    // --- Cập nhật TDS ---
    if (fabs(tds - prev_tds) >= 0.5) {
        int rect_w = frame_w - (text_x - frame_x); // đủ rộng dòng TDS
        int rect_h = 18;
        tft_fill_rect(text_x, start_y + 2*line_h - 1, rect_w, rect_h, bg_color);
    ...
    // --- Cập nhật TURB ---
    if (fabs(turb - prev_turb) >= 0.01) {
        int rect_w = frame_w - (text_x - frame_x); // đủ rộng dòng TURB
        int rect_h = 18;
        tft_fill_rect(text_x, start_y + 3*line_h - 1, rect_w, rect_h, bg_color);
