...
#define TURB_V_CLEAN 3.0     
#define TURB_V_DIRTY 2.5      
#define TURB_MAX_NTU 15.0
...
float sensor_read_turb(void)
{
    int sum = 0;
    for (int i = 0; i < 20; i++) {
        sum += adc1_get_raw(TURB_PIN);
    }

    float raw_avg = sum / 20.0;
    float v = to_voltage(raw_avg);
    ...
    ntu = fmaxf(0.0, fminf(ntu, TURB_MAX_NTU));

    ESP_LOGI(TAG, "TURB RawAvg: %.0f | Volt: %.3f | NTU: %.1f", raw_avg, v, ntu);
    return ntu;
}
